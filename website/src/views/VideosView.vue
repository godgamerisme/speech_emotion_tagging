      ...
      <div class="row">
        ...
        <div class="col-md-3 mt-5 offset-md-1"> 
          <div class="container border border-primary rounded py-4">
            ...
            <input v-model="searchQuery" placeholder="Search by patient name" class="form-control"/>
            
            <!-- Sorting criteria -->
            <div>
              <label for="sortCriteria" class="mt-4">Sort by:</label>
              <select id="sortCriteria" v-model="sortCriteria" class="form-select my-1">
                <option value="None">None</option>
                <option value="Date">Date</option>
                <option value="Age">Age</option>
              </select>
              <!-- Toggle button for sorting order -->
              <button v-if="sortCriteria !== 'None'" @click="toggleSortOrder" class="btn btn-primary my-1">
        ...
        <div class="col-md-8"> 
          <div class="thumbnail-grid">
            ...
            >
              <img :src="getThumbnailUrl(video.thumbnail_key)" :alt="video.metadata.patientName" class="thumbnail-image" />
...
export default {
  ...
  setup() {
    ...
    const filteredVideos = computed(() => {
      let filtered = videos.value.filter(video => video.metadata.patientName.toLowerCase().includes(searchQuery.value.toLowerCase()));
      ...
      } else if (sortCriteria.value === 'Age') { // Added sorting by age
        filtered = filtered.sort((a, b) => {
          if (sortOrder.value === 'ascending') {
